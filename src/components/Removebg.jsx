...
export default function RemoveBackground() {
    ...
    const handleRemoveBackground = async () => {
        const apiKey = "u68QgM8Fm8eyeAQSi7b8JEYo"
        const apiUrl = "https://api.remove.bg/v1.0/removebg"

        const formData = new FormData();
        formData.append("image_file", image, image.name); //this helps to fetch the selected image and size as its paramneter
        ...
        try {
            ...
            const data = await res.blob();//blob=binary large object, stores images, audios, multimedia files

            const reader = new FileReader();
            reader.onloadend = () => setBgRemove(reader.result)//removes bg
